...
Mellow Mushroom Custom s_code version 0.3 */


var s_d=document.domain; // could use path or location
if (s_d.indexOf("mellowdev.thesupergroup.com") > -1) {
	s_account = "mlwdevelopment";
} else {
	if (s_d.indexOf("mellowmushroom.com") > -1) {
		s_account = "mlwproduction";
	} else {
		if (s_d.indexOf("nutritionix.com") > -1) {
			s_account = "mlwproduction";
		} else {
				if (s_d.indexOf("mmbeerclub.com") > -1) {
					s_account = "mlwproduction";
				} else {
					var s_account = "mlwdevelopment";
			}
		}
	}
}
...
s.linkInternalFilters="javascript:mellowmushroom.com,mellowdev.thesupergroup.com,mmbeerclub.com"
...
function s_doPlugins(s){
    ...
    s.prop50="Mellow Mushroom 0.3 | SiteCatalyst code version: H.25.2";
    ...
    /*site section*/
    if(s.channel){s.eVar8 = "D=ch";}
   
    /*website*/
    s.eVar5 = document.location.hostname;
    s.prop6 = "D=v5";
   
    /*time parting*/
    s.eVar9 = s.getTimeParting("h","-5");
    s.eVar10 = s.getTimeParting("d","-5");
    s.eVar11 = s.getTimeParting("w","-5");
    ...
    /*visit information*/
    s.eVar12 = s.prop15 = s.getVisitNum(5000); // visit number
    s.eVar13 = "D=User-Agent"; // user agent
    s.eVar6 = s.prop7 = "D=g"; // page URL
   
    /*external campaigns*/
    if(!s.campaign){s.campaign = s.getQueryParam("cid");} // may change or be added to
    s.campaign = s.getValOnce(s.campaign, "s_campaign", 0);
    s.prop16 = s.getAndPersistValue(s.campaign, "s_prop16", 0); // campaign pathing
    if(s.prop16){s.prop16 = s.prop16 + ">" + s.pageName}

    s.prop10 = s.getPreviousValue(s.pageName, "s_prop10"); // previous page
    if(s.prop10){s.prop11 = s.getPercentPageViewed();} // percent page viewed
   
    s.prop8 = document.location.pathname.toString(); // auto page name
    s.prop9 = document.title.toLowerCase(); // title
   
    /*Internal campaign*/
    if(!s.eVar14){
        s.eVar14 = s.getQueryParam("icid");
    }
    ...
    /*copy variables*/
    if(s.eVar15){ s.prop17 = "D=v15";} // search term
...
s.visitorNamespace="mellowmushroom"
s.trackingServer="mellowmushroom.d2.sc.omtrdc.net"
...
+"rn 0};s.uns=function(){var s=this,x=s.dynamicAccountSelection,l=s.dynamicAccountList,m=s.dynamicAccountMatch,n,i;s.un=s.un.toLowerCase();if(x&&l){if(!m)m=s.wd.locaountMatch,n,i;s.un=s.un.toLowerCase();if(x&&l){if(!m)m=s.wd.location.host;if(!m.toLowerCase)m=''+m"
...
function s_giqf(){var w=window,q=w.s_giq,i,t,s;if(q)for(i=0;i<q.length;i++){t=q[i];s=s_gi(t.oun);s.sa